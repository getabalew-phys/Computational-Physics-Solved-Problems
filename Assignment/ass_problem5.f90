...
PROGRAM problem3_1a
    ...
    REAL :: x(0:N), y_c(0:N), z_c(0:N), dx, a=0, c=1.8
    INTEGER :: k, i
    ...
    OPEN(UNIT=10, FILE="ass_problem5.csv", POSITION="APPEND", ACCESS="SEQUENTIAL", STATUS="REPLACE")
    DO k=0, N-2
        WRITE(UNIT=10, FMT="(7f9.5)") x(k), y_c(k), z_c(k)
    END DO
    CLOSE(UNIT=10)
